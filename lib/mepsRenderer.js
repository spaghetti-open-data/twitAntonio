  ...
  formatAdditional: function(meps) {
    ...
    for (var i = 0; i < meps.length; i++) {
      ...
      if (typeof additionalProperties !== 'undefined') {
        ...
        // number formatting
        meps[i].mep_tweet_count_format = this.formatNumber(meps[i].mep_tweet_count);
        meps[i].mep_follower_count_format = this.formatNumber(meps[i].mep_follower_count);

        // riparte url
        if (meps[i].RiparteUrl) {
          meps[i].riparte = meps[i].RiparteUrl;
        }
